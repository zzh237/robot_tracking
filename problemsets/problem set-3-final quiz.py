...
class robot:
    ...
    def move(self, motion):
        ...
        if d < 0.0:
            raise ValueError, 'moving backwards not okay'

        if abs(alpha) > max_steering_angle:
            raise ValueError, "steering over the limit"
        ...
        if abs(beta) < 0.001:
            x = self.x + d2 * cos(self.orientation)
            y = self.y + d2 * sin(self.orientation)
        else:
            R = d2 / beta
    ...
    def sense(self, add_noise=1):
        ...
        return Z  # make sure your move function returns an instance
