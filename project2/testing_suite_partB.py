...
def execute_student_plan(warehouse, todo, max_distance, max_steering):
    ...
    student_planner = partB.DeliveryPlanner(warehouse, todo, max_distance, max_steering)
...
class State:
    ...
    def print_to_console(self):
        ...
        for b in self.boxes:
            box = self.boxes[b]
            print '\tbox id {}, x = {:6.2f}, y = {:6.2f}'.format(b,box['ctr_x'],box['ctr_y']) 
...
class PartBTestCase(unittest.TestCase):
    ...
    def run_test_with_params(self, k):
        ...
        errorMessage = ''
        try:
            cost,delivered = execute_student_plan(params['warehouse'], params['todo'], params['max_distance'],params['max_steering'])
            accomplished = True
        except Exception as e:
            errorMessage = repr(e)
        
        self.assertTrue(accomplished, errorMessage)
        return params['min_cost']/cost * float(len(delivered))/float(len(params['todo'])) 


    @timeout(TIME_LIMIT)
    def test_case1(self):
        score = self.run_test_with_params(1)
        print 'credit: {}'.format(score)

    @timeout(TIME_LIMIT)
    def test_case2(self):    
        score = self.run_test_with_params(2)
        print 'credit: {}'.format(score)
