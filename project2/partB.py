...
class DeliveryPlanner:
    ...
    def __init__(self, warehouse, todo, max_distance, max_steering):
        ...
        # test code only for running sample code below - do not use the following in your submittal
        rows = len(warehouse)
        ...
        # override the default test cases to run simple tests for collisions etc.
        # using the testcase 2 warehouse layout
        #self.testcase = 4
    ...
    def plan_delivery(self):
        ...
        # A non optimal plan for the first scenario
        if self.testcase == 1:
        ...
        # A non optimal plan for the second scenario            
        if self.testcase == 2:

           moves = [ 'move -1.570963 .0',       # a total of -135 degrees to the SE
                      'move -0.785398 1.414',    
                      'move -0.785398 3.0',    # move towards west
                      'move 1.570963 3.0',     # rotate and move south
                      'move 1.570963 2.0',     # rotate and move east
                      'move 1.570963 0.6',     # rotate north and move towards box 0
                      'lift 0',
                      'move 1.570963 0.0',     # rotate 180 degrees (to the south)
                      'move 1.570963 0.6',     # and move to the south
                      'move -1.570963 2.0',    # rotate and then move to the west
                      'move -1.570963 3.0',    # rotate and then move north
                      'move -1.570963 4.0',    # rotate and then move east
                      'move 1.570963 0.6',     # rotate and then move north
                      'down 5.5 -0.5',         # set box 0 in the delivery zone
                      'move 1.570963 0.0',     # turn a total of 135 degrees
                      'move 0.785398 .707',
                      'move -0.785398 4.5',    # move towards west
                      'lift 1',                # lift the remaining box
                      'move -1.570963 0.0',    # rotate back to the east
                      'move -1.570963 5.0',    # and move towards delivery zone
                      'move 1.570963 0.5',     # rotate and move north
                      'down 5.5 -0.5']         # set it down

        # Some test cases with penalty moves included (see comments with ***)
        if self.testcase == 3:
            moves = ['move 0.0 1.0',         # ***Try moving directly east - should crash into warehouse east wall  cost = 100
                     'move -1.570963 0.0',   # cost += 1 = 101
                     'move -1.570963 3.0']   # ***Rotated back west - crash into obstacle  cost += 100*3 = 401 

        if self.testcase == 4:
          moves = [ 'move -1.570963 .0',       # a total of -135 degrees to the SE   cost = 1.0
                      'move -0.785398 1.414',  #                                     cost += 2.414  = 3.414
                      'move -0.785398 3.0',    # move towards west                   cost += 4 = 7.414
                      'move 1.570963 3.0',     # rotate and move south               cost += 4 = 11.414
                      'move 1.570963 2.0',     # rotate and move east                cost += 3 = 14.414
                      'move 1.570963 0.9',     # *** rotate north, move too close to box 0             cost += 100*0.9 = 104.414
                      'move 1.570963 0.0',     # rotate west                         cost += 1 = 105.414
                      'move 1.570963 .5',      # rotate south and move away from box cost += 1.5 = 106.914
                      'lift 0',                # *** try to lift a box, too far away cost += 2 = 108.914
                      'move -1.570963 0.0',    # rotate west                         cost += 1 = 109.914
                      'move -1.570963 0.4',    # rotate north and move closer        cost += 1.4 = 111.314
                      'lift 0',                # legally lift box                    cost += 2 = 113.314
                      'down 2.5 -4.',          # *** try to set box down too far away                  cost += 1.5 = 114.814
                      'move 1.570963 0.',      # rotate west                         cost += 1 = 115.814
                      'move 1.570963 0.55',    # rotate south and move south         cost += 1.55 = 117.364
                      'move -1.570963 3.0',    # rotate west and move west           cost += 4.0 = 121.364
                      'move -1.570963 3.1',    # rotate north and move north         cost += 4.1 = 125.464
                      'down 0.4 -1.1']         # *** try to set down too close to another box          cost += 1.5 = 126.964
...
import robot
#
# robot = robot.Robot(x=2.0, y=-2.0, bearing=0.0,
#                     max_distance=5.0, max_steering=0.1)
#
# robot.move(0.2, 3, noise=False)

# x = ['abc', 'acc']
# print 'improperly formatted action: {}'.format(''.join(x))
# def chk(key):
#     try:
#         return x[key]
#     except Exception as e:
#         raise Exception('improperly formatted action: {}'.format(''.join(key)))
...
class PartBTestCase(unittest.TestCase):
    ...
    def run_test_with_params(self, k):
        ...
        params.update(GLOBAL_PARAMETERS[k])
        print "params", params
        accomplished = True
        errorMessage = True
